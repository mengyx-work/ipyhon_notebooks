...
class DataGenerator(object):
    ...
    def _prep_data(self):
        ...
        self._raw_file_list = sorted(glob.glob(os.path.join(self.data_path, "*.pkl")))
        if len(self._raw_file_list) == 0:
            raise ValueError('given data {} in `DataGenerator` {} is empty'.format(self.data_path, self.name))
        self._data_size()

    def _data_size(self):
        self.data_buffer, self.label_buffer, self.buffer_size = read_compressed_chunk_data(self._raw_file_list[0])
        self.frame_num = len(self.data_buffer[0])
        self.frame_size = len(self.data_buffer[0][0])
        if len(self._raw_file_list) > 1:
            _, _, tail_file_record_count = read_compressed_chunk_data(self._raw_file_list[-1])
        else:
            tail_file_record_count = self.buffer_size
        self.total_record_count = (len(self._raw_file_list) - 1) * self.buffer_size + tail_file_record_count

    def iter_num(self, batch_size):
        return int(math.ceil(1. * self.total_record_count / batch_size))

    def _load_file_by_index(self, file_index):
        self.data_buffer, self.label_buffer, self.buffer_size = read_compressed_chunk_data(self._raw_file_list[file_index])

    def _next_file_index(self):
        if self._cur_file_index >= len(self._raw_file_list) - 1:
    ...
    def _load_entire_data(self):
        ...
        ## only single file in entire data
        if file_index == 0:
            return
        while True:
            if file_index >= len(self._raw_file_list):
                break
            data_buffer, label_buffer, buffer_size = read_compressed_chunk_data(self._raw_file_list[file_index])
            file_index += 1
            self.data_buffer.extend(data_buffer)
            self.label_buffer.extend(label_buffer)
        return

    def generate_batch_iter(self, batch_size=32):
        if batch_size <= 0:
            self._load_entire_data()
            print('iterator created from {}, load entire data of {} records'.format(self.name, len(self.data_buffer)))
            while True:
                yield self.data_buffer, self.label_buffer
        ...
        while True:
            if self._cur_record_index + batch_size <= self.buffer_size:
                ...
                self._load_file_by_index(self._next_file_index())
